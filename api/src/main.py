...
@app.get("/location", response_model= List[Item], responses={404: {"model": Message}}, tags=["locations"])
async def read_loc():
    logger.info("logging from the /location")
    return data
    
    
@app.get("/location/{item_id}", response_model=Item, responses={404: {"model": Message}}, tags=["providers"])
async def read_item(item_id: str):
    logger.info("logging from the /location/{item_id}")
    for loc in data:
        logger.info("reading ID "+loc["loc_id"]+" in the data search logger for "+ item_id)
        
        if item_id == loc["loc_id"]:
            return {
            "loc_id": loc["loc_id"],
            "typ": loc["typ"],
            "name": loc["name"],
